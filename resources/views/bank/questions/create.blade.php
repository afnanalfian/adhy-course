...
@section('content')
    <form method="POST" action="{{ route('bank.material.questions.store', $material) }}" enctype="multipart/form-data"
        class="max-w-5xl mx-auto space-y-6">

        @csrf

        {{-- HEADER --}}
        <div class="bg-ens-lightest dark:bg-ens-darker rounded-xl shadow p-6">
            <h1 class="text-2xl font-bold text-secondary dark:text-ens-lighter">
        ...
        {{-- TEST TYPE --}}
        <div class="bg-ens-lightest dark:bg-ens-darker rounded-xl shadow p-6 space-y-2">
            <label class="font-semibold dark:text-ens-lighter">Jenis Tes</label>
            <select id="test-type" name="test_type" class="w-full rounded-lg border p-2">
        ...
        {{-- TIPE SOAL --}}
        <div
            class="bg-ens-lightest text-secondary dark:bg-ens-darker dark:text-ens-lighter rounded-xl shadow p-6 space-y-2">
            <label class="font-semibold">Tipe Soal</label>
            <select id="question-type" name="type" class="w-full rounded-lg border p-2
                           bg-ens-lightest dark:bg-secondary/40
        ...
        {{-- SOAL --}}
        <div
            class="bg-ens-lightest text-secondary dark:bg-ens-darker dark:text-ens-lighter rounded-xl shadow p-6 space-y-4">
            <h2 class="text-lg font-semibold">Soal</h2>

            <textarea id="question-text" name="question_text" rows="4" class="w-full rounded-lg border p-3
                             bg-ens-lightest dark:bg-secondary/30
                             text-slate-800 dark:text-white" placeholder="Tulis soal di sini..."></textarea>

            <input type="file" name="question_image" class="text-sm">

            <button type="button"
                class="btn-open-math px-4 py-2 bg-secondary text-white dark:bg-ens-lighter dark:text-secondary rounded-lg"
            ...
            <div class="mt-4">
                ...
                <div id="question-preview" class="prose dark:prose-invert
                            max-w-none
                            p-4 rounded-lg
        ...
        <div id="options-section"
            class="bg-ens-lightest text-secondary dark:bg-ens-darker dark:text-ens-lighter rounded-xl shadow p-6 space-y-4 hidden">
            <h2 class="text-lg font-semibold">Opsi Jawaban</h2>

            <div id="options-wrapper" class="space-y-4"></div>

            <button type="button" id="add-option" class="px-3 py-1 rounded bg-primary text-white">
                + Tambah Opsi
            </button>
        </div>
            ...
            <div id="short-answers-wrapper" class="space-y-3">
                <div class="short-answer-item flex items-center gap-3 p-3 border rounded-lg">
                    <input type="text" name="short_answers[0][text]"
                        class="flex-1 rounded-lg border p-2 bg-ens-lightest dark:bg-secondary/30 text-slate-800 dark:text-white"
        ...
        <div
            ...
            <textarea id="explanation-text" name="explanation" rows="4" class="w-full rounded-lg border p-3
                            bg-ens-lightest dark:bg-secondary/30
                            text-slate-800 dark:text-white"
            ...
            <div class="mt-4">
                ...
                <div id="explanation-preview" class="prose dark:prose-invert
                            max-w-none
                            p-4 rounded-lg
                            bg-slate-50 dark:bg-secondary/30
                            border border-slate-200 dark:border-white/10">
        ...
        <div class="flex justify-end gap-3">
            ...
            {{-- BATAL --}}
            <button type="submit" form="cancel-form" class="px-6 py-2 rounded-lg
            ...
            {{-- SIMPAN --}}
            <button type="submit" class="px-6 py-2 bg-green-400 text-black rounded-lg hover:bg-green-700 hover:text-white">
                Simpan Soal
            </button>
    ...
    {{-- FORM BATAL --}}
    <form id="cancel-form" method="GET" action="{{ url()->previous() }}" class="sweet-confirm"
        data-message="Yakin ingin batal? Semua data soal yang anda buat akan hilang">
    </form>

    {{-- MATH MODAL --}}
    <div id="math-modal" class="fixed inset-0 z-50 hidden
                bg-black/40">

        <div class="absolute bottom-6 left-1/2 -translate-x-1/2
                    w-full max-w-xl
                    bg-ens-lightest dark:bg-secondary/90
                    border border-slate-200 dark:border-white/10
            ...
            <div class="flex justify-between items-center">
                ...
                <button id="close-math-modal" class="text-slate-500 hover:text-red-500">
                    ✕
                </button>
            </div>

            <div id="math-editor" class="border rounded-lg p-3 min-h-[70px] text-lg
            ...
            <div class="flex justify-between pt-2">
                <div class="flex gap-2">
                    <button id="btn-confirm-math" type="button" class="px-4 py-1.5 rounded-lg
                                   bg-primary text-white
                                   hover:bg-ens-darker">
                        Tambahkan
                    </button>

                    <button id="btn-cancel-math" type="button" class="px-4 py-1.5 rounded-lg
                ...
                <button id="btn-open-docs" type="button" class="text-sm dark:text-white underline opacity-80">
                    📘 Dokumentasi
                </button>
            </div>
        </div>
