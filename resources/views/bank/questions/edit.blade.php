...
@section('content')
    ...
    <form method="POST"
        ...
        enctype="multipart/form-data" class="max-w-5xl mx-auto space-y-6">
        @csrf
        ...
        | TIPE TEST (EDIT)
        ========================= --}}
        <div class="bg-ens-lightest dark:bg-ens-darker rounded-xl p-4 shadow-sm">
            <label class="block text-sm font-semibold mb-1">
                Tipe Test
            </label>

            <select id="test-type" class="w-full rounded-lg border p-2
        ...
        <div
            ...
            <select id="question-type" name="type" class="w-full rounded-lg border p-2
                ...
                <option value="mcq" {{ old('type', $question->type ?? '') == 'mcq' ? 'selected' : '' }}>Pilihan Ganda (1
                    Benar)</option>
                <option value="mcma" {{ old('type', $question->type ?? '') == 'mcma' ? 'selected' : '' }}>Pilihan Ganda
                    (Banyak Benar)</option>
                <option value="truefalse" {{ old('type', $question->type ?? '') == 'truefalse' ? 'selected' : '' }}>Benar /
                    Salah</option>
                <option value="short_answer" {{ old('type', $question->type ?? '') == 'short_answer' ? 'selected' : '' }}>
                    Isian Singkat</option>
                <option value="compound" {{ old('type', $question->type ?? '') == 'compound' ? 'selected' : '' }}>Soal
                    Kompleks</option>
            </select>
        </div>
