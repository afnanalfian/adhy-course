...
@section('content')
    <div x-data="{ openAddQuestion: false }" class="max-w-6xl mx-auto space-y-8">
        ...
        {{-- ================= FORM ================= --}}
        <div class="bg-ens-lightest dark:bg-secondary/80
                    rounded-2xl p-6
            ...
            <form method="POST" action="{{ route('exams.update', $exam) }}" class="space-y-6">
                ...
                @if(in_array($exam->type, ['quiz', 'tryout']))
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title', $exam->title) }}" class="w-full rounded-lg
                                        border-gray-300 dark:border-white/10
                                        bg-white dark:bg-secondary
                                        dark:text-white
                                        focus:ring-primary focus:border-primary" @disabled($exam->status !== 'inactive')>
                    </div>
                @endif
                ...
                <div>
                    ...
                    <div class="w-full sm:w-64 rounded-lg
                            px-3 py-2
                ...
                @if(in_array($exam->type, ['quiz', 'tryout']))
                            <div>
                                ...
                                <input type="datetime-local" name="exam_date" value="{{ old(
                        'exam_date',
                ...
                <div>
                    ...
                    <input type="number" name="duration_minutes" min="1"
                        value="{{ old('duration_minutes', $exam->duration_minutes) }}" class="w-full sm:w-64 rounded-lg
                                border-gray-300 dark:border-white/10
                                bg-white dark:bg-secondary
                                dark:text-white
                                focus:ring-primary focus:border-primary" @disabled($exam->status !== 'inactive')>
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <a href="{{ $exam->backRoute() }}" class="px-4 py-2 rounded-lg
