...
@section('content')
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
            @php
                ...
                $stats = array_merge($stats, [
                    [
                        ...
                        'value' => $attempt->score,
                        'icon' => 'star',
                        'color' => 'text-ens-medium dark:text-white',
                        'bg' => 'bg-ens-lightest dark:bg-ens-medium'
                    ],
                    [
                        'label' => 'Durasi',
            ...
            @foreach ($stats as $stat)
                <div class="rounded-xl border border-ens-lighter dark:border-ens-darker
                                bg-white dark:bg-ens-darker p-4 shadow-sm">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-ens-lighter dark:divide-ens-medium">
                            ...
                            <tbody class="bg-white dark:bg-ens-darker divide-y divide-ens-lighter dark:divide-ens-medium">
                                {{-- BENAR --}}
                                <tr class="hover:bg-ens-lightest/50 dark:hover:bg-ens-medium/20">
                                    <td class="px-6 py-4">
                                ...
                                {{-- SCORE --}}
                                <tr
                                    class="bg-ens-lightest/30 dark:bg-ens-medium/10 hover:bg-ens-lightest/50 dark:hover:bg-ens-medium/20">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2">
                                            <div class="w-2 h-2 rounded-full bg-ens-medium dark:bg-white"></div>
                                            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Skor</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <span class="text-lg font-bold text-ens-darkest dark:text-white">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <a href="{{ route('exams.ranking.student', $exam) }}" class="inline-flex items-center justify-center gap-2
                      px-5 py-2.5 rounded-lg text-sm font-semibold
                      bg-gradient-to-r from-ens-medium to-ens-light text-white
                      hover:from-ens-light hover:to-ens-medium
            ...
            <form method="GET" class="flex items-center gap-3">
                ...
                <select name="per_page" onchange="this.form.submit()" class="text-sm rounded-lg border border-ens-lighter dark:border-ens-medium
                               bg-white dark:bg-ens-darker text-gray-700 dark:text-gray-200
                               px-3 py-2 focus:outline-none focus:ring-2 focus:ring-ens-light focus:border-transparent">
        ...
        <div class="space-y-6">
            @foreach ($questions as $examQuestion)
                ...
                <div class="rounded-xl border border-ens-lighter dark:border-ens-darker
                                bg-white dark:bg-ens-darker p-6 hover:shadow-sm transition-shadow">

                    {{-- HEADER SOAL --}}
                    <div class="flex items-center justify-between mb-6">
                        <div class="flex items-center gap-3">
                            <div
                                class="w-10 h-10 rounded-lg bg-ens-lightest dark:bg-ens-medium flex items-center justify-center">
                                <span class="font-bold text-ens-medium dark:text-white">
                                    #{{ $questionNumber }}
                                </span>
                            </div>
                        ...
                        <div class="text-sm text-gray-500 dark:text-gray-400">
                            @if($answer)
                                @if(in_array($question->options->where('is_correct', true)->first()->id ?? null, $selected))
                                    <span
                                        class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300">
                                ...
                                @else
                                    <span
                                        class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300">
                                        <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd"
                            ...
                            @else
                                <span
                                    class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300">
                                    <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd"
                    ...
                    <div class="mb-6 space-y-4">
                        @if ($question->image)
                            <div class="border border-ens-lighter dark:border-ens-medium rounded-lg overflow-hidden">
                    ...
                    <div class="space-y-3 mb-6">
                        @foreach ($question->options as $option)
                            ...
                            : ($isChosen
                                ? 'border-red-400 bg-gradient-to-r from-red-50 to-white dark:from-red-900/20 dark:to-ens-darker shadow-sm'
                                : 'border-ens-lighter dark:border-ens-darker bg-white dark:bg-ens-darker hover:border-ens-light dark:hover:border-ens-light'
                            ) }}">
                            ...
                            : ($isChosen
                                ...
                                : 'bg-ens-lightest dark:bg-ens-medium text-ens-medium dark:text-white border border-ens-lighter dark:border-ens-medium'
                            ) }}">
                                                {{ $option->label }}
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex-1">
                                            @if ($hasImage)
                                                <div
                                                    class="mb-3 border border-ens-lighter dark:border-ens-medium rounded-lg overflow-hidden max-w-xs">
                                            ...
                                            <div class="flex flex-wrap items-center gap-3 mt-3">
                                                ...
                                                @if ($isChosen)
                                                    <span
                                                        class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-300">
                                                        <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                                            <path fill-rule="evenodd"
                    ...
                    @if ($question->explanation)
                        <div x-data="{ open: false }" class="border-t border-ens-lighter dark:border-ens-medium pt-5">
                            <button @click="open = !open"
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="font-medium text-ens-darkest dark:text-white">
                                        Pembahasan Soal
                                    </span>
                                </div>
                            ...
                            <div x-show="open" x-collapse class="mt-4 prose prose-sm dark:prose-invert max-w-none dark:text-white">
