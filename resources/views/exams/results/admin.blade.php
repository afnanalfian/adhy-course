        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
            ...
            @foreach ($stats as $stat)
                <div class="rounded-xl border border-ens-lighter dark:border-ens-darker
                                bg-ens-lightest dark:bg-ens-darker p-5 shadow-sm">
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="text-sm text-gray-500 dark:text-gray-400 mb-1">
                                {{ $stat['label'] }}
                            </div>
                            <div class="text-2xl font-bold text-ens-darkest dark:text-white">
                                {{ $stat['value'] }}
                            </div>
                        </div>
                        <div class="p-2 bg-ens-light dark:bg-ens-medium rounded-lg">
                            @if($stat['icon'] === 'users')
                                <svg class="w-6 h-6 text-ens-lightest dark:text-white" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
        ...
        <div class="mb-12">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                ...
                <form method="GET" class="flex items-center gap-3">
                    ...
                    <select name="rank_per_page" onchange="this.form.submit()" class="text-sm rounded-lg border border-ens-lighter dark:border-ens-medium
                                bg-ens-lightest dark:bg-ens-darker text-gray-700 dark:text-gray-200
            ...
            <div class="rounded-xl border border-ens-lighter dark:border-ens-darker overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-ens-lighter dark:divide-ens-medium">
                        <thead class="bg-ens-darker dark:bg-ens-darkest">
                            <tr>
                                <th scope="col"
                                    class="px-6 py-3 text-left text-xs font-semibold text-white dark:text-gray-300 uppercase tracking-wider">
                                    Peringkat
                                </th>
                                <th scope="col"
                                    class="px-6 py-3 text-left text-xs font-semibold text-white dark:text-gray-300 uppercase tracking-wider">
                                ...
                                @if ($exam->type === 'tryout' && $exam->test_type === 'skd')
                                    <th scope="col"
                                        class="px-4 py-3 text-center text-xs font-semibold text-white dark:text-gray-300 uppercase tracking-wider">
                                        TWK
                                    </th>
                                    <th scope="col"
                                        class="px-4 py-3 text-center text-xs font-semibold text-white dark:text-gray-300 uppercase tracking-wider">
                                ...
                                @endif
                                <th scope="col"
                                    class="px-6 py-3 text-center text-xs font-semibold text-white dark:text-gray-300 uppercase tracking-wider">
                                    Total
                                </th>
                        ...
                        <tbody
                            class="bg-ens-lightest dark:bg-ens-darker divide-y divide-ens-lighter dark:divide-ens-medium">
                            @forelse ($ranking as $attempt)
                                <tr class="hover:bg-ens-lightest/50 dark:hover:bg-ens-medium/20 transition-colors">
                                    ...
                                    @if (!($exam->type === 'tryout' && $exam->test_type === 'skd'))
                                        <td class="px-4 py-4 text-center">
                                            <span
                                                class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 font-semibold text-sm">
                                                {{ $attempt->correct }}
                                            </span>
                                        </td>
                                        ...
                                        <td class="px-4 py-4 text-center">
                                            <span
                                                class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 font-semibold text-sm">
                                                {{ $attempt->wrong }}
                                            </span>
                                        </td>
                                        ...
                                        @if ($exam->test_type === 'mtk_stis')
                                            <td class="px-4 py-4 text-center">
                                                <span
                                                    class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 font-semibold text-sm">
                                                    {{ $attempt->empty }}
                                                </span>
                                            </td>
                                        @endif
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <span class="text-lg font-bold text-ens-medium dark:text-white">
                                            {{ $attempt->score }}
                                        </span>
                                    </td>
        ...
        <div>
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                ...
                <form method="GET" class="flex items-center gap-3">
                    ...
                    <select name="per_page" onchange="this.form.submit()"
                        class="text-sm rounded-lg border border-ens-lighter dark:border-ens-medium
                                   bg-ens-lightest dark:bg-ens-darker text-gray-700 dark:text-gray-200
                                   px-3 py-2 focus:outline-none focus:ring-2 focus:ring-ens-light focus:border-transparent">
                        @foreach ([10, 20, 30, 50, 100] as $n)
            ...
            <div class="space-y-4">
                @foreach ($questions as $examQuestion)
                    ...
                    <div class="rounded-xl border border-ens-lighter dark:border-ens-darker
                                    bg-ens-lightest dark:bg-ens-darker p-5 hover:shadow-md transition-shadow">
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div class="flex-1">
                                <div class="flex flex-wrap items-center gap-3 mb-2">
                                    <div class="font-bold text-lg text-ens-darkest dark:text-white">
                            ...
                            <a href="{{ route('exams.questions.analysis', [$exam, $examQuestion]) }}" class="inline-flex items-center justify-center gap-2
