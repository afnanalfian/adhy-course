    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            <form method="GET" action="{{ route('course.index') }}" class="flex w-full sm:w-auto gap-2">
                <input type="text" name="q" placeholder="Cari course..." value="{{ $q ?? '' }}" class="flex-1 px-3 py-2 rounded-xl border border-gray-300 dark:border-gray-700
        ...
        @forelse($courses as $c)
            <article class="group bg-ens-lightest dark:bg-ens-darker rounded-2xl shadow-md overflow-hidden
                ...
                <a href="{{ route('course.show', $c->slug) }}" class="block">
                    ...
                    <div class="relative">
                        <img src="{{ $c->thumbnail ? asset('storage/' . $c->thumbnail) : asset('img/course-default.png') }}"
                            alt="{{ $c->name }}" class="w-full h-40 sm:h-44 object-cover">
                ...
                <div
                    ...
                    <a href="{{ route('course.edit', $c->slug) }}" class="px-3 py-1.5 rounded-lg text-sm bg-ens-medium/10 text-ens-darker dark:text-ens-lighter
                                hover:bg-ens-medium/20 transition">
                        Edit
                    </a>

                    {{-- DELETE WITH SWEETALERT --}}
                    <form method="POST" action="{{ route('course.delete', $c->slug) }}" class="sweet-confirm"
