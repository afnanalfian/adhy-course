...
@section('content')
    ...
    <div class="max-w-3xl mx-auto px-4 py-8">
        ...
        {{-- TITLE --}}
        <h1 class="text-2xl font-bold text-ens-darker dark:text-ens-lighter mb-6">
            Edit Course
        </h1>
        ...
        <form action="{{ route('course.update', $course->slug) }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $course->name) }}" class="w-full mt-1 px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-700
                              bg-white dark:bg-ens-darkest text-gray-800 dark:text-gray-100
                              focus:ring-2 focus:ring-primary/40 focus:outline-none" required>
            </div>
            {{-- SLUG --}}
            <div>
                <label class="block font-semibold text-sm text-gray-700 dark:text-gray-200">
                ...
                <input type="text" name="slug" value="{{ old('slug', $course->slug) }}" class="w-full mt-1 px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-700
                            bg-white dark:bg-ens-darkest text-gray-800 dark:text-gray-100
                            focus:ring-2 focus:ring-primary/40 focus:outline-none" required>
            ...
            <div>
                ...
                <textarea name="description" rows="5" class="w-full mt-1 px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-700
                                 bg-white dark:bg-ens-darkest text-gray-800 dark:text-gray-100
            ...
            {{-- NEW THUMBNAIL --}}
            <div class="p-4 border border-gray-300 dark:border-gray-700 rounded-xl bg-white dark:bg-ens-darkest/50">

                <label class="block font-semibold text-sm text-gray-700 dark:text-gray-200">
                    Ganti Thumbnail
                </label>

                <input type="file" name="thumbnail" class="mt-3 text-sm text-gray-700 dark:text-gray-200
            ...
            <div class="pt-4">
                <button class="px-5 py-2.5 rounded-xl bg-primary text-white
                               hover:bg-ens-medium transition shadow-md">
                    Update Course
                </button>
            </div>
