...
<div x-data="{ open: {{ $userActive ? 'true' : 'false' }} }" class="relative">
    ...
    {{-- Submenu --}}
    <div x-show="open" x-transition @click.outside="open = false" class="ml-6 mt-2 space-y-1 border-l
               border-ens-medium/30
               dark:border-ens-light/20 pl-4">
        <a href="{{ route('siswa.index') }}" class="menu-subitem {{ request()->routeIs('siswa.*') ? 'active' : '' }}">
            <span class="w-1 h-1 rounded-full bg-current"></span>
            Siswa
        </a>

        <a href="{{ route('tentor.index') }}" class="menu-subitem {{ request()->routeIs('tentor.*') ? 'active' : '' }}">
            <span class="w-1 h-1 rounded-full bg-current"></span>
            Tentor
        </a>
...
<div x-data="{ open: {{ $purchaseActive ? 'true' : 'false' }} }" class="relative">
    ...
    <div x-show="open" @click.outside="open = false" x-transition class="ml-6 mt-2 space-y-1 border-l
        ...
        <a href="{{ route('orders.index') }}" class="menu-subitem {{ request()->routeIs('orders.*') ? 'active' : '' }}">
            <span class="w-1 h-1 rounded-full bg-current"></span>
            Order Masuk
...
{{-- ================= BANK SOAL ================= --}}
<a href="{{ route('bank.category.index') }}" class="menu-item {{ request()->routeIs('bank.*') ? 'active' : '' }}">
    <svg class="menu-icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"
        stroke-width="1.5">
        <path stroke-linecap="round" stroke-linejoin="round"
