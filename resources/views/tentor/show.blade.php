...
@section('content')
    <div class="min-h-screen">
        ...
        <div class="max-w-2xl mx-auto">
            ...
            {{-- Back --}}
            <a href="{{ route('tentor.index') }}" class="inline-flex items-center gap-2 text-sm font-medium mb-4
            ...
            <div class="rounded-2xl shadow-lg border border-gray-200/70 dark:border-gray-700/50
                        bg-ens-lightest dark:bg-ens-darker p-5 sm:p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm mb-6">
                    <div class="p-3 rounded-lg bg-white dark:bg-ens-darkest/40">
                ...
                <div class="mt-8 grid grid-cols-2 sm:flex sm:flex-row gap-3">
                    ...
                    {{-- WA --}}
                    <a href="https://wa.me/{{ $teacher->user->whatsapp_phone }}" target="_blank" class="flex items-center justify-center px-3 py-2 rounded-lg
                        bg-green-600 hover:bg-green-700 text-white text-sm w-full text-center">
                        WhatsApp
                    </a>

                    {{-- EDIT --}}
                    <a href="{{ route('tentor.edit', $teacher->id) }}" class="flex items-center justify-center px-3 py-2 rounded-lg
                        bg-primary hover:bg-primary/90 text-white text-sm w-full text-center">
                        Edit
                    </a>

                    {{-- TOGGLE ACTIVE --}}
                    <form method="POST" action="{{ route('tentor.toggle', $teacher->id) }}" class="w-full sweet-confirm"
                    ...
                    {{-- REMOVE --}}
                    <form method="POST" action="{{ route('tentor.remove', $teacher->id) }}" class="w-full sweet-confirm"
