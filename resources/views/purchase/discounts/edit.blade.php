...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <form method="POST"
          action="{{ route('purchase.discounts.update', $discount) }}"
          class="space-y-6
        ...
        <div>
            ...
            <select name="type"
                ...
                <option value="percentage" @selected($discount->type === 'percentage')>
                    Persentase (%)
                </option>
                <option value="fixed" @selected($discount->type === 'fixed')>
                    Nominal (Rp)
                </option>
            </select>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="starts_at"
                       value="{{ optional($discount->starts_at)->format('Y-m-d') }}"
                       class="mt-1 w-full rounded-xl border-gray-300
                              dark:bg-azwara-darkest dark:border-azwara-darker">
            </div>
            ...
            <div>
                ...
                <input type="date" name="ends_at"
                       value="{{ optional($discount->ends_at)->format('Y-m-d') }}"
                       class="mt-1 w-full rounded-xl border-gray-300
                              dark:bg-azwara-darkest dark:border-azwara-darker">
            </div>
        </div>
