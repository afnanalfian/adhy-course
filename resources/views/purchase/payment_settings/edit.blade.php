...
@section('content')
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <form method="POST" action="{{ route('payment.settings.update') }}" enctype="multipart/form-data" class="space-y-6 p-6 rounded-2xl border
            ...
            <div>
                ...
                @if($settings['qris_image'] ?? null)
                    <img src="{{ asset('storage/' . $settings['qris_image']) }}" class="mt-3 max-w-xs rounded-xl border">
                @endif

                <input type="file" name="qris_image" accept="image/*" class="mt-3 block w-full text-sm
            ...
            <div>
                ...
                <textarea name="payment_instruction" rows="4"
                    class="mt-1 w-full rounded-xl border-gray-300 dark:text-white bg-ens-lightest
            ...
            <div>
                ...
                <select name="active_payment_method" class="w-full rounded-xl bg-ens-lightest border-gray-300 dark:text-white
                               dark:bg-ens-darkest dark:border-ens-darker
                               focus:ring-primary focus:border-primary">

                    <option value="manual_qris" @selected(($settings['active_payment_method'] ?? '') === 'manual_qris')>
                        Manual QRIS
                    </option>
