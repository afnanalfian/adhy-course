...
@section('content')
    <div class="max-w-7xl mx-auto space-y-6 px-4 sm:px-6 lg:px-0">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <a href="{{ route('pricing.create') }}" class="inline-flex items-center justify-center
                    ...
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            ...
                            <p class="text-xs mt-1 uppercase
                                    ...
                                    bg-gray-100 dark:bg-ens-darker
                ...
                : 'bg-gray-100 text-gray-600' }}">
                    ...
                    <div class="grid grid-cols-2 gap-3 text-sm text-gray-700 dark:text-gray-300">
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-gray-900 dark:text-ens-lightest">
                    ...
                    <div class="flex items-center justify-end gap-4 pt-2 text-sm">
                        <a href="{{ route('pricing.edit', $rule) }}" class="text-primary font-semibold hover:underline">
                            Edit
                        </a>

                        {{-- HAPUS (hanya jika nonaktif) --}}
                        @if(!$rule->is_active)
                            <form method="POST" action="{{ route('pricing.destroy', $rule) }}" class="sweet-confirm"
                        ...
                        @endif
                        <form method="POST" action="{{ route('pricing.toggle', $rule) }}" class="sweet-confirm"
                ...
                <tbody class="divide-y dark:divide-ens-darker">
                    @forelse($rules as $rule)
                                <tr class="align-top">
                                    ...
                                    <td class="px-6 py-4 font-semibold text-gray-900 dark:text-ens-lightest">
                                        Rp {{ number_format($rule->price, 0, ',', '.') }}
                                    </td>
                        ...
                        : 'bg-gray-100 text-gray-600' }}">
                                    ...
                                    <td class="px-6 py-4 text-right space-x-3 whitespace-nowrap">
                                        ...
                                        {{-- HAPUS (hanya jika nonaktif) --}}
                                        @if(!$rule->is_active)
                                            <form method="POST" action="{{ route('pricing.destroy', $rule) }}" class="inline sweet-confirm"
                                                data-message="Yakin ingin menghapus pricing rule ini?">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        <form method="POST" action="{{ route('pricing.toggle', $rule) }}" class="inline sweet-confirm"
                                            data-message="Yakin ingin mengubah status pricing rule ini?">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="font-semibold
