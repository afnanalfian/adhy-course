@if($activePromo && $activePromo->isCurrentlyActive() && $activePromo->type == 'banner')
    ...
    <div id="promoBanner" class="fixed top-0 left-0 right-0 z-[90] transition-all duration-300"
        data-delay="{{ $activePromo->display_delay }}" data-duration="{{ $activePromo->display_duration }}"
        data-promo-id="{{ $activePromo->id }}">

        <div class="relative bg-gradient-to-r from-primary to-ens-darker text-white shadow-lg">
            <!-- Close Button -->
