@if($activePromo && $activePromo->isCurrentlyActive() && $activePromo->type == 'modal')
    <div id="promoModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 hidden"
        ...
        <div class="relative z-10 w-full max-w-md bg-white rounded-xl shadow-2xl animate-modalIn">
            <!-- Modal Header -->
            <div class="p-6 border-b border-ens-lighter">
                <div class="flex justify-between items-start">
                    <div>
                        @if($activePromo->title)
                            <h3 class="text-lg font-bold text-ens-darkest mb-1">{{ $activePromo->title }}</h3>
                        @endif
            ...
            <div class="p-6">
                @if($activePromo->image_url)
                    <div class="relative w-full flex items-center justify-center bg-ens-lightest rounded-lg">
                ...
                <div class="flex flex-col sm:flex-row gap-3">
                    @if($activePromo->target_url)
                        <a href="{{ $activePromo->resolved_url }}" target="_blank"
                            class="flex-1 px-4 py-3 bg-primary text-white font-medium rounded-lg hover:bg-ens-medium transition-colors text-center">
                            Lihat Promo
                        </a>
                    @endif

                    <button id="modalCancel"
                        class="flex-1 px-4 py-3 border border-ens-lighter text-ens-darkest font-medium rounded-lg hover:bg-ens-lightest transition-colors">
                        Nanti Saja
                    </button>
                </div>
            </div>
            ...
            <!-- Footer -->
            <div class="px-6 py-3 bg-ens-lightest rounded-b-xl border-t border-ens-lighter">
                <p class="text-xs text-gray-500 text-center">
                    Promo ini berlaku hingga
                    {{ $activePromo->ends_at ? $activePromo->ends_at->format('d M Y') : 'waktu yang ditentukan' }}
                </p>
            </div>
        </div>
    </div>
