        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                <div class="flex h-12 w-12 items-center justify-center rounded-lg bg-ens-lighter/30 dark:bg-ens-darker">
                    <svg class="h-6 w-6 text-ens-medium dark:text-ens-lighter" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="overflow-hidden rounded-xl border border-ens-lighter bg-white dark:border-gray-700 dark:bg-gray-800">
        ...
        <div class="grid grid-cols-7 divide-x divide-y divide-ens-lighter dark:divide-gray-700">
            @for ($date = $start->copy(); $date <= $end; $date->addDay())
                ...
                <div class="min-h-[140px] p-3 {{ !$isCurrentMonth ? 'bg-ens-lightest/30 dark:bg-gray-900/30' : '' }} {{ $isWeekend ? 'bg-ens-lightest/50 dark:bg-gray-900/20' : '' }}">
                    {{-- Date Header --}}
                    <div class="mb-2 flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <span class="text-sm font-medium {{ $isToday ? 'flex h-7 w-7 items-center justify-center rounded-full bg-ens-medium text-white' : 'text-ens-darker dark:text-white' }}">
                                {{ $date->day }}
                            </span>
                            @if($isToday)
                        ...
                        @if($eventCount > 0)
                            <span class="rounded-full bg-ens-lightest px-2 py-0.5 text-xs font-medium text-ens-medium dark:bg-gray-700 dark:text-gray-300">
                    ...
                    <div class="space-y-1.5">
                        ...
                        {{-- Empty State --}}
                        @if($eventCount === 0)
                            <div class="flex h-16 items-center justify-center rounded-lg border border-dashed border-ens-lighter dark:border-gray-600">
